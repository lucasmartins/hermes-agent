...
class TestProviderSelectionGate:
    ...
    def test_explicit_qwen3_sees_dotenv_endpoint(self):
        from tools import transcription_tools as tt
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
             ...
             patch.object(tt, "_has_local_command", return_value=False), \
             patch("hermes_cli.config.load_env",
                   return_value={"QWEN3_ASR_BASE_URL": "127.0.0.1:8002"}):
            assert tt._get_provider({"enabled": True, "provider": "qwen3"}) == "qwen3"
...
class TestTranscribeCallSitesReadDotenv:
    ...
    def test_transcribe_qwen3_forwards_dotenv_config(self):
        from tools import transcription_tools as tt
        ...
        class FakeOpenAIClient:
            def __init__(self, *, api_key=None, base_url=None, timeout=None, max_retries=None):
                ...
                self.audio = MagicMock()
                self.audio.transcriptions.create.return_value = {"text": "hello qwen"}
                captured["create"] = self.audio.transcriptions.create
            def close(self):
                pass

        def fake_get_env_value(name, default=None):
            return {
                "QWEN3_ASR_BASE_URL": "127.0.0.1:8002",
                "QWEN3_ASR_API_KEY": "qwen-dotenv-key",
        ...
        with patch.object(tt, "get_env_value", side_effect=fake_get_env_value), \
             ...
             patch.dict("sys.modules", {"openai": fake_openai_module}), \
             patch("builtins.open", MagicMock()):
            result = tt._transcribe_qwen3("/tmp/fake.mp3", "Qwen/Qwen3-ASR-1.7B")

        assert result["success"] is True
        assert captured["api_key"] == "qwen-dotenv-key"
        assert captured["base_url"] == "http://127.0.0.1:8002/v1"
        create_kwargs = captured["create"].call_args.kwargs
